/******************************************************************************/
// FreeRTOS example developed by Erle Robotics(2018).
// This example use the cubeMX HAL and the LWIP library.
// This is an example of implementation of a TCP/IP echo server
//
/*****************************************************************************/
/* Includes ------------------------------------------------------------------*/
#include "main.h"
...
void StartDefaultTask(void const * argument)
{
  ...
  //Listening to that connection
  netconn_listen(conn);

  for(;;)
  {
	  //Accepting new in connection
	  err = netconn_accept(conn, &newconn);
	  if (err == ERR_OK) {
		  struct netbuf *buf;
		  void *data;
		  uint16_t len;
		  //If we receive new data and are OK
